...
from scripts.external_eval_adapter import build_external_eval_report, write_external_eval_outputs
...
class FinTraceCoreTest(unittest.TestCase):
    def test_batch_graph_exports_trace_and_decisions(self) -> None:
        ...
        self.assertEqual(result["batch_metrics"]["case_count"], data["case_count"])
    ...
    def test_showcase_helpers_build_storyline_pairs_and_comparison(self) -> None:
        root = test_root("showcase_helpers")
        dataset = Path(__file__).resolve().parents[1] / "datasets" / "showcase_fintrace_v1"
        result = run_batch([str(dataset)], output_root=root / "runs", batch_id="showcase-helper", max_workers=1)
        ground_truth = dataset / "ground_truth.json"

        storyline = build_showcase_storyline(result, ground_truth)
        scenario_counts = {row["scenario"]: row["案件数"] for row in storyline["scenarios"]}
        self.assertEqual(
            scenario_counts,
            {
                "time_space_conflict": 2,
                "split_invoice_evasion": 3,
                "purpose_attachment_mismatch": 1,
            },
        )
        self.assertTrue(all(row["命中正确"] == row["案件数"] for row in storyline["scenarios"]))

        pairs = suggest_showcase_pairs(result, ground_truth)
        pair_ids = {(pair["left_case_id"], pair["right_case_id"]) for pair in pairs}
        self.assertIn(("SHOW-TS-01", "SHOW-TS-02"), pair_ids)
        self.assertIn(("SHOW-SPLIT-01", "SHOW-MIS-01"), pair_ids)

        cases = {case["case_id"]: case for case in result["case_results"]}
        comparison = build_case_comparison(cases["SHOW-TS-01"], cases["SHOW-TS-02"])
        self.assertIn("机票", comparison["left"]["next_action"])
        self.assertTrue(any(row["字段"] == "金额" for row in comparison["field_rows"]))
        self.assertTrue(any(row["字段"] == "amount" and row["左侧来源"] for row in comparison["provenance_rows"]))
        self.assertIn("R012_TIME_SPACE_CONFLICT", {row["左规则ID"] for row in comparison["rule_rows"]})

    def test_showcase_state_helpers_detect_identity_and_inconsistent_prefixes(self) -> None:
        root = test_root("showcase_state")
        dataset = Path(__file__).resolve().parents[1] / "datasets" / "showcase_fintrace_v1"
        result = run_batch([str(dataset)], output_root=root / "runs", batch_id="showcase-state", max_workers=1)

        self.assertEqual(case_prefix_summary(result), {"SHOW": 6})
        self.assertFalse(validate_active_result_consistency(result, "showcase_fintrace_v1"))
        self.assertEqual(detect_dataset_identity([str(dataset)], result)["dataset"], "showcase_fintrace_v1")
        self.assertEqual(detect_dataset_identity([r"D:\03_AI_Projects\FinTrace\datasets\fintrace-redteam-v1"])["dataset"], "fintrace-redteam-v1")
        self.assertEqual(detect_dataset_identity([r"D:\03_AI_Projects\FinTrace\datasets\red_attack_v1"])["dataset"], "red_attack_v1")
        self.assertEqual(detect_dataset_identity([r"D:\03_AI_Projects\FinTrace\runtime\demo\demo_001"])["dataset"], "dynamic_redteam")
        ...
        warnings = validate_active_result_consistency(mixed_result, "showcase_fintrace_v1")
        self.assertTrue(any("非 SHOW" in warning or "SHOW-" in warning for warning in warnings))
        self.assertTrue(any("多个 case 前缀" in warning for warning in warnings))

    def test_clear_review_widget_state_removes_batch_sensitive_keys(self) -> None:
        state = {
            "finance_decision_filter_showcase-1": ["人工复核"],
            "finance_case_detail": "old-case",
            "diagnostic_case_selector_showcase-1": "SHOW-TS-01",
            "comparison_left_showcase-1": "SHOW-TS-01",
            "feedback_reason_SHOW-TS-01": "人工确认",
            "deepseek_api_key": "keep-me",
        }
        removed = clear_review_widget_state(state)

        self.assertIn("finance_decision_filter_showcase-1", removed)
        self.assertIn("finance_case_detail", removed)
        self.assertIn("diagnostic_case_selector_showcase-1", removed)
        self.assertIn("comparison_left_showcase-1", removed)
        self.assertIn("feedback_reason_SHOW-TS-01", removed)
        self.assertEqual(state, {"deepseek_api_key": "keep-me"})
    ...
    def test_external_eval_adapter_reads_batch_artifacts_and_emits_framework_samples(self) -> None:
        root = test_root("external_eval_adapter")
        data = generate_redteam_batch(root / "data", n=16, seed=19)
        result = run_batch([data["source_dir"]], output_root=root / "runs", batch_id="external-eval", max_workers=1)
        batch_result = Path(result["work_dir"]) / "batch_result.json"

        report = build_external_eval_report(batch_result)
        metrics = report["agent_eval_metrics"]

        self.assertEqual(report["source"]["case_count"], 17)
        self.assertEqual(metrics["task_completion_rate"], 1.0)
        self.assertEqual(metrics["tool_call_accuracy"], 1.0)
        self.assertGreaterEqual(metrics["trace_explainability_score"], 0.9)
        self.assertEqual(len(report["ragas_agent_samples"]), 17)
        self.assertEqual(len(report["deepeval_agent_cases"]), 17)

        output_paths = write_external_eval_outputs(report, root / "external_eval")
        self.assertTrue(Path(output_paths["report"]).exists())
        self.assertTrue(Path(output_paths["ragas_samples"]).exists())
        self.assertTrue(Path(output_paths["deepeval_cases"]).exists())
