...
def generate_redteam_batch(output_dir: str | Path, n: int = 80, seed: int = 42) -> dict[str, Any]:
    ...
    # Track if we need to inject split companion for the next split_invoice scenario
    split_companion_injected = False

    for i in range(n):
        scenario = SCENARIOS[i % len(SCENARIOS)]

        # Inject the split companion row right BEFORE a split_invoice scenario lands
        # so both cases fall into the same batch_features split group
        if scenario == "split_invoice" and not split_companion_injected:
            companion = build_split_companion(i, rng, split_vendor, split_date, seed)
            rows.append(companion)
            labels.append(build_label(companion, "split_invoice"))
            write_invoice_attachment(attachments / f"{companion['reimbursement_id']}_发票OCR.txt", companion, "split_invoice")
            write_chat_attachment(attachments / f"{companion['reimbursement_id']}_审批聊天.md", companion, "split_invoice")
            split_companion_injected = True
    ...
    return {"source_dir": str(root), "erp_path": str(erp_path), "ground_truth_path": str(label_path), "case_count": actual_count}
...
def build_split_companion(
    ...
    """Generate a companion row for split_invoice scenario so split_group_count >= 2.

    Companion reuses employee, vendor, date, and expense_type as the primary
    split_invoice row but with a different invoice_no and amount.
    """
    # Must match the primary split_invoice row's E003 so batch_features groups them together
    employee = EMPLOYEES[2]  # E003 / 张悦 / 市场增长部
    client = CLIENTS[i % len(CLIENTS)]
    amount = round(rng.uniform(800, 1200), 2)
    claim = f"FT-SPLIT-CP-{i:05d}"
    invoice_no = f"SPLITCP{20260500 + (i % 28) + 1}{rng.randint(100000, 999999)}"
    return {
        "reimbursement_id": claim,
        "报销单号": claim,
        "employee_id": employee["employee_id"],
        ...
        "发票号码": invoice_no,
        "invoice_code": f"04{rng.randint(1000000000, 9999999999)}",
        "invoice_hash": f"HASH-SPLIT-CP-{claim}",
        "invoice_type": "增值税电子普通发票",
        "client_id": client["client_id"],
        "client_name": client["client_name"],
        "has_original_invoice": True,
