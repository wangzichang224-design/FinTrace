...
MAX_ZIP_FILE_SIZE = 500 * 1024 * 1024       # 500 MB max total zip size
MAX_ZIP_EXTRACTED_FILES = 10_000             # max files per zip
MAX_DIR_SCANNED_FILES = 10_000               # max files per directory scan
...
def prepare_sources(source_paths: list[str], work_dir: Path) -> list[Path]:
    ...
    for raw in source_paths:
        ...
        if path.is_dir():
            ...
            if len(files) > MAX_DIR_SCANNED_FILES:
                raise RuntimeError(
                    f"目录 {path} 包含超过 {MAX_DIR_SCANNED_FILES} 个文件，"
                    f"请缩小批次规模。"
                )
            concrete.extend(files)
            continue
        if path.suffix.lower() == ".zip":
            if path.stat().st_size > MAX_ZIP_FILE_SIZE:
                raise RuntimeError(
                    f"ZIP 文件 {path.name} ({path.stat().st_size / 1024 / 1024:.0f} MB) "
                    f"超过上限 {MAX_ZIP_FILE_SIZE / 1024 / 1024:.0f} MB"
                )
            target = ensure_dir(work_dir / "unzipped" / path.stem)
            with zipfile.ZipFile(path, "r") as zf:
                # Zip Slip protection: validate all paths are within target
                for info in zf.infolist():
                    resolved = os.path.normpath(os.path.join(str(target), info.filename))
                    if not resolved.startswith(str(target)):
                        zf.close()
                        raise RuntimeError(
                            f"ZIP 文件 {path.name} 包含路径遍历攻击：{info.filename}"
                        )
                # Resource limit: cap extracted file count
                if len(zf.infolist()) > MAX_ZIP_EXTRACTED_FILES:
                    raise RuntimeError(
                        f"ZIP 文件 {path.name} 包含 {len(zf.infolist())} 个文件，"
                        f"超过上限 {MAX_ZIP_EXTRACTED_FILES}"
                    )
        ...
        if path.stat().st_size > MAX_FILE_SIZE:
            raise RuntimeError(
                f"文件 {path.name} ({path.stat().st_size / 1024 / 1024:.0f} MB) "
                f"超过上限 {MAX_FILE_SIZE / 1024 / 1024:.0f} MB"
            )
